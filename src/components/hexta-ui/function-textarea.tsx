...
import { Textarea } from "@/components/hexta-ui/textarea"
	...
	return (
		<div className="relative flex h-full flex-col">
			<Textarea
				...
				onChange={onChange}
				className={cn(
					"min-h-[200px] w-full flex-1 resize-none rounded-xl border-2 font-code text-sm shadow-lg transition-all duration-300 hover:shadow-xl focus:shadow-2xl",
					className
				)}
				readOnly={readOnly}
				wrapLines={wrapLines}
			...
			<div className="absolute top-1 right-1 flex gap-1">
				{showWrapToggle && (
					<Button
						...
						variant="ghost"
						className={cn(
							"size-10 text-muted-foreground hover:text-foreground transition-all duration-300 hover:scale-110",
							wrapLines && "text-primary bg-primary/10"
						)}
						onClick={() => setWrapLines(!wrapLines)}
						title={wrapLines ? "Switch to horizontal scroll" : "Wrap long lines"}>
						{wrapLines ? <ScrollText className="size-10" /> : <WrapText className="size-10" />}
					</Button>
				)}
				{!readOnly && (
					<Button
						size="icon"
						variant="ghost"
						className="size-10 text-muted-foreground hover:text-foreground transition-all duration-300 hover:scale-110"
						onClick={handlePaste}
						title="Paste">
