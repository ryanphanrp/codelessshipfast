		...
		<Card>
			<CardHeader className="pb-3">
				<div className="flex items-center justify-between">
					...
					<OutputActions hasOutput={hasContent} onCopy={onCopy} onClear={onClear} />
				</div>
			</CardHeader>
			<CardContent>
				{error ? (
							...
							<ScrollArea className="h-[350px] w-full">
								<div className="space-y-2">
									{envVariables.map((envVar, index) => (
										<EnvVariableItem key={index} envKey={envVar.key} envValue={envVar.value} />
									))}
								</div>
							</ScrollArea>
						)}
						{displayMode === "property-lines" && parsedPropertyLines.length > 0 && (
							<ScrollArea className="h-[350px] w-full">
								<div className="space-y-1">
									{parsedPropertyLines.map((line, index) => (
										<PropertyLineItem key={index} line={line} lineNumber={index + 1} />
									))}
								</div>
							</ScrollArea>
						)}
				...
				{isConverting && <p className="mt-2 text-muted-foreground text-sm">Converting...</p>}
			</CardContent>
		</Card>
	)
})
